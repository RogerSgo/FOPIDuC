...
static void motor(uint32_t trab){	// Funcion que permite actualizar el valor de ul_duty
...
/* *************** Codigo de aplicacion *************** */
	...
	{		
		...
		snprintf(cad3, sizeof(cad3), "%.0f", (double)setp);
		gfx_draw_string(cad3, 140, 60, &sysfont, GFX_COLOR(0, 0, 0), GFX_COLOR(0, 255, 0));
		...
		float yk = (float)vel_red;		// lectura de la senal de velocidad del sensor (encoder)
		float ek = setp - yk;		// senal de error: setp - yk
		...
		for (uint32_t i = 1U; i <= CONTROLLER_ORDER; i++){
			sal += b[i] * error_history[i];
			sal -= a[i] * output_history[i];
		}

		/* Conservar internamente la salida calculada; limitar solo la senal
		 * aplicada al actuador al intervalo valido del PWM. */
		float actuator_output = sal;
		if (actuator_output > (float)PERIOD_VALUE){
			actuator_output = (float)PERIOD_VALUE;
		}
		if (actuator_output < 0.0f){
			actuator_output = 0.0f;
		}

		for (uint32_t i = CONTROLLER_ORDER; i > 1U; i--){
			error_history[i] = error_history[i - 1U];
			output_history[i] = output_history[i - 1U];
		}
		error_history[1] = ek;
		output_history[1] = sal;
		//uk_6=uk_5;uk_5=uk_4;uk_4=uk_3;uk_3=uk_2;uk_2=uk_1;uk_1=sal;
		//ek_6=ek_5;ek_5=ek_4;ek_4=ek_3;ek_3=ek_2;ek_2=ek_1;ek_1=ek;		

		snprintf(cad5, sizeof(cad5), "%.2f", (double)ek);
		snprintf(cad6, sizeof(cad6), "%.2f", (double)sal);
		gfx_draw_string(cad5, 105, 85, &sysfont, GFX_COLOR(0, 0, 0), GFX_COLOR(0, 255, 0));
		gfx_draw_string(cad6, 130, 95, &sysfont, GFX_COLOR(0, 0, 0), GFX_COLOR(0, 255, 0));
		motor((uint32_t)actuator_output);		// enviar senal PWM al puerto de salida
		printf("%" PRIu32 "\n", vel_red);
		/*
